...
def gen_graphs(n):
    ...
    for i in range(n):
        deg = int(no_v / 5)
        # deg = 2
        g = nx.random_regular_graph(n = no_v, d = deg)
        # g = nx.gnp_random_graph(no_v, 0.5)
        g.name = f"graph_{i}"
...
for no_v in test_v_size:
    ...
    signatures = {}
    for g in graphs:
        signature = nx.weisfeiler_lehman_graph_hash(g, iterations=5)
        if signature not in signatures:
            signatures[signature] = []
        signatures[signature].append(g)
...
plt.subplot(1, 2, 1)
plt.plot([x * (x / 2) / 2 for x in test_v_size], average_time, label="LSH Hashing")
plt.plot([x * (x / 2) / 2 for x in test_v_size], average_time_wf, label="WFL Hash")
plt.xlabel('|E|')
...
plt.subplot(1, 2, 2)
plt.plot([x * (x - 1) / 2 * 0.6 for x in test_v_size], false_pos, label="LSH Hashing",  marker='o')
plt.plot([x * (x - 1) / 2 * 0.6 for x in test_v_size], false_pos_wf, label="WFL Hash", marker='x')
plt.xlabel('|E|')
